...
const clubRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/', async (request, reply) => {
    ...
    let query = supabase.from('clubs').select('*').order('member_count', { ascending: false });
    ...
    return reply.send({ success: true, data: (data ?? []).map(mapClub), error: null } satisfies ApiResponse<Club[]>);
  });

  /** GET /api/v1/clubs/:slug */
  fastify.get<{ Params: { slug: string } }>('/:slug', async (request, reply) => {
    const { data, error } = await supabase.from('clubs').select('*').eq('slug', request.params.slug).maybeSingle();
    if (error) {
      fastify.log.error(error);
      return reply.status(500).send({ success: false, data: null, error: 'Database error' });
    }
    if (!data) return reply.status(404).send({ success: false, data: null, error: 'Club not found' });
    return reply.send({ success: true, data: mapClub(data), error: null } satisfies ApiResponse<Club>);
  });
};
