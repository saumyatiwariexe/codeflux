...
const CATEGORIES = ['electronics', 'bag', 'wallet', 'id_card', 'keys', 'clothing', 'books', 'other'];
const lostItemSchema = zod_1.z.object({
    title: zod_1.z.string().min(1).max(120),
    description: zod_1.z.string().min(1).max(1000),
    category: zod_1.z.enum(CATEGORIES),
    lastSeenLocation: zod_1.z.string().optional(),
...
const foundItemSchema = zod_1.z.object({
    ...
    description: zod_1.z.string().min(1).max(1000),
    category: zod_1.z.enum(CATEGORIES),
    foundLocation: zod_1.z.string().optional(),
    foundAt: zod_1.z.string().datetime().optional(),
    imageUrl: zod_1.z.string().url().optional(),
});
function mapLost(row) {
    return {
        id: row.id,
...
const lostfoundRoutes = async (fastify) => {
    fastify.get('/lost', async (_req, reply) => {
        const { data, error } = await supabase_1.supabase.from('lost_items').select('*').order('reported_at', { ascending: false });
        if (error) {
            fastify.log.error(error);
            return reply.status(500).send({ success: false, data: null, error: 'Database error' });
        }
        return reply.send({ success: true, data: (data ?? []).map(mapLost), error: null });
    });
    fastify.get('/found', async (_req, reply) => {
        const { data, error } = await supabase_1.supabase.from('found_items').select('*').order('reported_at', { ascending: false });
        if (error) {
            fastify.log.error(error);
            return reply.status(500).send({ success: false, data: null, error: 'Database error' });
        }
        return reply.send({ success: true, data: (data ?? []).map(mapFound), error: null });
    });
    fastify.post('/lost', { preHandler: auth_1.requireAuth }, async (request, reply) => {
        ...
        return reply.status(201).send({ success: true, data: mapLost(data), error: null });
    });
    fastify.post('/found', { preHandler: auth_1.requireAuth }, async (request, reply) => {
        const { userId } = request.user;
        ...
        return reply.status(201).send({ success: true, data: mapFound(data), error: null });
    });
    /** GET /api/v1/lostfound/matches — heuristic category + text-overlap match, real data, no ML. */
    fastify.get('/matches', { preHandler: auth_1.requireAuth }, async (_req, reply) => {
        const [{ data: lost, error: lostErr }, { data: found, error: foundErr }] = await Promise.all([
            supabase_1.supabase.from('lost_items').select('*').eq('status', 'open'),
            supabase_1.supabase.from('found_items').select('*').eq('status', 'unclaimed'),
        ]);
        if (lostErr || foundErr) {
...
exports.default = lostfoundRoutes;
